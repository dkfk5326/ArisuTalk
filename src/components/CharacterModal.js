...
function renderStickerGrid(app, stickers) {
    ...
    return stickers.map((sticker, index) => {
        ...
        return `
            <div class="sticker-item relative group ${isSelected && isSelectionMode ? 'ring-2 ring-blue-500' : ''}" ${isSelectionMode ? `data-index="${index}"` : ''}>${isSelectionMode ? `
                        ...
                        <div class="content-wrapper">
                            <div class="content-inner pt-6 space-y-6">
                                    ...
                                    <div class="content-wrapper">
                                        <div class="content-inner pt-4 space-y-4">
                                            <div class="flex items-center justify-between mb-3">
                                                ...
                                                <div class="flex items-center gap-2">
                                                    <button id="toggle-sticker-selection" class="py-2 px-3 bg-gray-700 hover:bg-gray-600 text-white rounded-lg transition-colors text-sm flex flex-col items-center gap-1" data-selection-mode="${app.state.stickerSelectionMode ? 'true' : 'false'}">
                                                        <i data-lucide="check-square" class="w-4 h-4"></i> 
                                                        <span class="toggle-text text-xs">${app.state.stickerSelectionMode ? '선택<br>해제' : '선택<br>모드'}</span>
                                                    </button>
                                                    ${app.state.stickerSelectionMode ? `
                                            ...
                                            <div class="flex items-center justify-between text-xs text-gray-500 mb-3">
                                                <span>전체 저장 용량: ${formatBytes(getLocalStorageUsage())}</span>
                                                <span>총 용량: ${formatBytes(app.calculateCharacterStickerSize(editingCharacter || {}))}</span>
                                            </div>
                                            <div id="sticker-container" class="grid grid-cols-4 gap-2">
                                                ${renderStickerGrid(app, editingCharacter?.stickers || [])}
                                            </div>
                                        </div>
                                    </div>
                                </details>
                                    ...
                                    <div class="content-wrapper">
                                        <div class="content-inner pt-4 space-y-2">
                                            <div id="memory-container" class="space-y-2">
                                                ${char.memories.map((mem, index) => renderMemoryInput(mem, index)).join('')}
                                            </div>
                                ...
                                <details class="group border-t border-gray-700 pt-2">
                                    ...
                                    <div class="content-wrapper">
                                        <div class="content-inner pt-4 space-y-4">
                                            ${renderSlider('responseTime', language.characterModalSlider.responseTime.description, language.characterModalSlider.responseTime.low, language.characterModalSlider.responseTime.high, char.responseTime)}
                                            ${renderSlider('thinkingTime', language.characterModalSlider.thinkingTime.description, language.characterModalSlider.thinkingTime.low, language.characterModalSlider.thinkingTime.high, char.thinkingTime)}
                                            ${renderSlider('reactivity', language.characterModalSlider.reactivity.description, language.characterModalSlider.reactivity.low, language.characterModalSlider.reactivity.high, char.reactivity)}
                                            ${renderSlider('tone', language.characterModalSlider.thinkingTime.description, language.characterModalSlider.thinkingTime.low, language.characterModalSlider.thinkingTime.high, char.tone)}
                                        </div>
                                    </div>
                                </details>
                            </div>
