...
import { loadFromLocalStorage, saveToLocalStorage, getLocalStorageUsage, checkStorageSpace } from './storage.js';
...
class PersonaChatApp {
    ...
    async migrateChatData() {
        const migrationCompleted = await loadFromLocalStorage('personaChat_migration_v16', false);
        ...
        this.state.characters.forEach(character => {
            ...
            if (newChatRooms[characterId] && newChatRooms[characterId].length > 0) return;
    ...
    async handleStickerFileSelect(e) {
        ...
        for (let i = 0; i < files.length; i++) {
            ...
            if (file.size > 30 * 1024 * 1024) {
                this.showInfoModal("파일 크기 초과", `${file.name}은(는) 30MB를 초과합니다.`);
                continue;
            }
        ...
        if (newStickers.length > 0) {
            ...
            const updatedCharacterData = { ...currentEditing, stickers: updatedStickers };
            
            const characterDataString = JSON.stringify(updatedCharacterData);
            const storageCheck = checkStorageSpace(characterDataString, 'personaChat_characters_v16');
    ...
    handleDeleteSticker(index) {
        ...
        const currentEditing = this.state.editingCharacter || {};
        this.setState({ editingCharacter: { ...currentEditing, stickers: updatedStickers } });
    }
    ...
    handleEditStickerName(index) {
        if (this.state.editingCharacter && this.state.editingCharacter.stickers) {
            ...
            if (newName !== null && newName.trim() !== '') {
                ...
                this.setState({ editingCharacter: { ...this.state.editingCharacter, stickers: newStickers } });
            }
        }
    }
    ...
    updateStickerSection() {
        ...
        if (toggleButton) {
            ...
            if (textSpan) textSpan.innerHTML = this.state.stickerSelectionMode ? '선택<br>해제' : '선택<br>모드';
        }
    ...
    handleStickerSelection(index, isChecked) {
        ...
        let newSelected = isChecked ? [...currentSelected, index] : currentSelected.filter(i => i !== index);
    ...
    handleSelectAllStickers() {
        ...
        document.querySelectorAll('.sticker-checkbox').forEach(cb => cb.checked = true);
    ...
    handleDeleteSelectedStickers() {
        ...
        this.state.editingCharacter = { ...this.state.editingCharacter, stickers: updatedStickers };
        this.state.selectedStickerIndices = [];
    ...
    async triggerApiCall(currentMessagesState, isProactive = false, isReroll = false, forceSummary = false) {
        ...
        if (isProactive) {
            ...
            chatId = characterChatRooms.length > 0 ? characterChatRooms[0].id : this.createNewChatRoom(character.id);
        } else {
    ...
    async checkAndSendProactiveMessages() {
        if (this.state.isWaitingForResponse || !this.state.settings.apiKey || !this.state.settings.proactiveChatEnabled) return;
    ...
    calculateCharacterStickerSize(character) {
        ...
        return character.stickers.reduce((total, sticker) => total + (sticker.size || 0), 0);
    }
    ...
    encodeTextInImage(imageData, text) {
        ...
        if (textLength > availableDataPixels) {
            this.showInfoModal(language.modal.imageTooSmallOrCharacterInfoTooLong.title, language.modal.imageTooSmallOrCharacterInfoTooLong.message);
            return null;
        }
    ...
    decodeTextFromImage(imageData) {
        ...
        if (data[3] !== 0x50 || data[7] !== 0x43 || data[11] !== 0x41 || data[15] !== 0x52) return null;

        const textLength = (data[19] << 24) | (data[23] << 16) | (data[27] << 8) | data[31];
        if (textLength <= 0 || textLength > (data.length / 4) - headerSizeInPixels) return null;
            ...
            try {
                ...
                if (backupData.settings && backupData.characters && backupData.messages && backupData.unreadCounts) {
                    this.showConfirmModal(
                        ...
                        () => {
                            saveToLocalStorage('personaChat_settings_v16', backupData.settings);
    ...
    handleRestorePrompts(e) {
        ...
        reader.onload = (event) => {
            try {
                ...
                if (restoredPrompts.main && restoredPrompts.profile_creation && typeof restoredPrompts.main.system_rules === 'string') {
                    this.showConfirmModal(
                        language.modal.promptRestoreConfirm.title,
                        language.modal.promptRestoreConfirm.message,
                        () => {
                            const newPrompts = {
                                main: { ...this.defaultPrompts.main, ...(restoredPrompts.main || {}) },
                                profile_creation: restoredPrompts.profile_creation || this.defaultPrompts.profile_creation
                            };
                            this.setState({ settings: { ...this.state.settings, prompts: newPrompts } });
                        }
                    );
                } else {
        ...
        this.showConfirmModal(
            ...
            () => {
                import('./defauts.js').then(({ defaultPrompts }) => {
                    ...
                    let textareaId = section === 'main' ? `prompt-main-${key}` : 'prompt-profile_creation';
                    const textarea = document.getElementById(textareaId);
                    if (textarea) {
                        textarea.value = section === 'main' ? defaultPrompts.main[key] : defaultPrompts.profile_creation;
                    }
